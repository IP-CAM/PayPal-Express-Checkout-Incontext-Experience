...
class ControllerModulePPincontext extends Controller {
	public function index() {
		...
		if ($status) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/pp_incontext.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/pp_incontext.tpl', $data);
			} else {
				return $this->load->view('default/template/module/pp_incontext.tpl', $data);
			}
		}
	}
}